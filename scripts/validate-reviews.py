...
recorded_fits = {t['id']: t.get('landmarks', []) for t in json.loads((ROOT / 'generated/registration-report.json').read_text())['transforms']}
for review in landmark_reviews['reviews']:
    ...
    for item in review['landmarks']:
        ...
        if review.get('transform_id') in recorded_fits:
            assert any(all(item.get(k) == p.get(k) for k in ('landmark', 'side', 'source_point_m', 'target_point_m'))
                       for p in recorded_fits[review['transform_id']]), item
        else:
