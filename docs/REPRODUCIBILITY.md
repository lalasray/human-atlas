...
python3 scripts/refresh-female-expansion.py # retain the committed MOOSE extension
```
...
composition 0.5: Denver at identity, the same-donor CT labels at identity (Denver bones and
...
The dev server or a static server of `dist/` (`npm run build && (cd dist && python3 -m http.server 3017 --bind 127.0.0.1)`) must listen on port 3017. In another terminal, run `.venv/bin/python scripts/browser-check.py`. Install Chromium with `.venv/bin/python -m playwright install chromium`, or set
`CHROMIUM_EXECUTABLE_PATH` to an existing browser binary. The script writes screenshots and canvas-pixel evidence to
`artifacts/browser/`; it exercises the three active references, source provenance, the registration
review panel with its landmark overlay, and the base catalog filters.
