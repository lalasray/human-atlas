...
This is an implementation of `docs/plans/female-open-human-atlas-plan.md` up to unified composition 0.5
...
- Unified experimental composition 0.5: 1,015 meshes represented as 933 canonical concepts, with 3,219,937 triangles and about 40 MB gzip. Exact duplicate structures use one winning source in donor-aware order: Denver VHF, then same-donor NLM CT, then HRA reference detail; TCIA remains a different-donor alternative. Multi-piece structures such as renal calyces retain every source fragment but appear as one selectable concept. Missing HRA laterality encoded in `_L`/`_R` asset identifiers is recovered, preventing bilateral anatomy from being mistaken for duplicates. The geometry and transforms remain composition 0.4's Denver (128), NLM CT (101), and HRA (786): 229 meshes from the VHF donor and 786 from the HRA assembly. HRA organ-proxy RMS is 7.42 mm; anatomy remains unreviewed.
