...
import {FEMALE_GAPS,FEMALE_SOURCES,INFANT_SOURCE,INFANT_THORAX_SOURCE,type ReferenceId} from './references';
...
export function PartSources({parts,expanded}:{parts:Part[];expanded:boolean}) {
  ...
  return <div className="part-sources"><h3>Source anatomy</h3>{sources.map(id=>{
    const source=id==='dhcp-neonatal'?INFANT_SOURCE:id==='tyndall-newborn-thorax'?INFANT_THORAX_SOURCE:FEMALE_SOURCES[id];
...
function ProjectOverview() {
  return <>
    <p>Human Atlas is a 3D anatomy explorer maintained by lalasray, with three models:</p>
    <ul>
      <li><strong>Male</strong> — adult BodyParts3D reference anatomy.</li>
      <li><strong>Female</strong> — expanded anatomy combining HRA, Denver and NLM sources.</li>
      <li><strong>Infant</strong> — brain and chest parts together in one scene.</li>
    </ul>
    <p>Switch models, search for a structure, show or hide systems, isolate parts, or use the explosion slider to explore the visible anatomy.</p>
    <a href="https://github.com/lalasray/human-atlas" target="_blank" rel="noreferrer">Project repository <ArrowUpRight size={14}/></a>
    <h3>Current model: coverage and sources</h3>
  </>;
}
...
export function AtlasSources({atlas,reference}:{atlas:Atlas|null;reference:ReferenceId}) {
  ...
  if(reference==='infant')return <div className="about-copy">
    <ProjectOverview/>
    <p><strong>Infant · Brain and chest</strong><br/>{atlas?.parts.length??'…'} parts in one view: 85 brain regions and nine chest organ and tissue groups.</p>
    <h3>Included anatomy</h3><ul><li>Brain regions, including the brainstem, cortex and deep brain structures.</li><li>Heart, lungs, trachea and an artery region.</li><li>Chest bones, cartilage, muscles, fat and skin.</li></ul>
    <h3>Combined for display</h3><p>The brain is uniformly resized and positioned above the chest. These models come from different sources, so their relative size and placement are approximate. This is a composite reference, not a scan of one infant or a validated anatomical alignment.</p>
    <p>The dHCP brain represents a mixed-sex population at 40 weeks post-menstrual age. The Tyndall chest comes from an infant born at 36 weeks gestation; sex and age at the scan are not reported.</p>
    <h3>Still missing</h3><p>Skull, face, neck, limbs, pelvis, abdominal and pelvic organs, and complete peripheral nerves and vessels. Bones and muscles remain grouped, and fine organ structures are not separately labeled.</p>
    <h3>Credits</h3><p>Brain: Schuh et al., Developing Human Connectome Project, CC BY 4.0. Chest: Andrea Pacheco, Baptiste Jayet, and Stefan Andersson-Engels, Tyndall National Institute (2021), newborn thorax mesh v1.0.0, CC BY 4.0. Local surface conversion and combined placement remain unreviewed.</p>
    <a href={INFANT_SOURCE.url} target="_blank" rel="noreferrer">dHCP brain source <ArrowUpRight size={14}/></a>
    <a href={INFANT_THORAX_SOURCE.url} target="_blank" rel="noreferrer">Tyndall chest source <ArrowUpRight size={14}/></a>
    <a href="/infant-sources/assembly.json" target="_blank" rel="noreferrer">Combined coverage and display transforms <ArrowUpRight size={14}/></a>
    <a href="/infant-sources/thorax/coverage.json" target="_blank" rel="noreferrer">Chest conversion checks <ArrowUpRight size={14}/></a>
    <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noreferrer">CC BY 4.0 license <ArrowUpRight size={14}/></a>
  ...
  return <div className="about-copy">
    ...
    {female?<>
      ...
      <p>Hand meshes group the carpal, metacarpal and finger bones on each side. Radius and ulna references are partial because the source scan cuts off parts of the arms; the right ulna is fragmented.</p>
