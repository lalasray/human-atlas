export type SystemId = 'skeletal'|'muscular'|'arterial'|'venous'|'nervous'|'digestive'|'respiratory'|'urinary'|'reproductive'|'lymphatic'|'endocrine'|'integumentary'|'connective'|'sensory'|'cardiac'|'pregnancy'|'tissue';
export const SYSTEMS: {id:SystemId;name:string;color:string;description:string}[] = [
 ...
 {id:'reproductive',name:'Reproductive',color:'#bda098',description:'Reproductive organs participate in gamete production, hormone secretion, and reproduction. The selected reference determines the anatomy represented.'},
 {id:'pregnancy',name:'Pregnancy reference',color:'#bd7f96',description:'Placental and umbilical reference structures are separate from the default adult anatomy.'},
 {id:'tissue',name:'CT tissue maps',color:'#759a91',description:'Grouped muscle and fat segmentations, shown as optional tissue depots. These masks do not resolve individual muscles or tissue layers.'},
...
export interface Provenance {id:string;structure_id:string;source:string;source_asset:string;source_url:string;source_revision:string;source_chunk_sha256:string;source_sex:string;reference_sex:string;source_donor:string;geometry_type:string;granularity?:string;ontology_mapping?:string;ontology_term_label?:string|null;display_space:string;canonical_space:string|null;registration:{type:string;display_transform_id:string;transform_id:string|null;rms_mm?:number|null;canonical_registration?:boolean;review_status?:string};confidence:number|null;confidence_basis:string;license:string;license_url:string;qa_status:string;notes:string;alternatives:string[];geometry_qa?:{boundary_edges:number;nonmanifold_edges:number;degenerate_faces:number;self_intersections:string;anatomical_review:string;connected_components?:number;outlier_components?:number}}
export interface Part {id:string;name:string;conceptId:string;system:SystemId;chunk:number;positions:number;normals:number;indices:number;vertexCount:number;indexCount:number;bounds:[number[],number[]];provenance?:Provenance}
export interface Concept {id:string;name:string;elements:string[]}
export interface LandmarkPair {landmark:string;side:string;source_point_m:number[];target_point_m:number[];residual_mm:number;source_assets?:string[];target_assets?:string[];source_concept?:string;target_labels?:string[]}
...
export interface Atlas {version:string;sex?:'male'|'female';source?:string;scope?:string;region?:'whole-body'|'brain'|'thorax'|'infant-assembly';developmental_stage?:string;reference_age?:string;default_visible?:SystemId[];parts:Part[];concepts:Concept[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];triangles:number;canonical_space?:string;registration_report?:{transforms:RegistrationTransform[]}}
...
export const EXPLANATIONS:Record<string,string> = {
 ...
 'vagina':'A muscular canal connecting the cervix with the exterior of the body.',
 'ovary':'An organ containing oocytes that also produces reproductive hormones.',
