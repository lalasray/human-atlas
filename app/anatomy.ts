...
export const SYSTEMS: {id:SystemId;name:string;color:string;description:string}[] = [
 ...
 {id:'reproductive',name:'Reproductive',color:'#bda098',description:'Reproductive structures contribute to reproduction and sex hormone production. The structures shown depend on the selected reference anatomy.'},
 ...
 {id:'pregnancy',name:'Pregnancy reference',color:'#c79baf',description:'Placental and umbilical reference structures are shown separately from the adult anatomy and are hidden by default.'},
...
export interface Part {id:string;name:string;conceptId:string;system:SystemId;chunk:number;positions:number;normals:number;indices:number;vertexCount:number;indexCount:number;bounds:[number[],number[]];provenance?:{source:string;source_asset:string;source_donor:string;geometry_type:string;license:string}}
export interface Concept {id:string;name:string;elements:string[]}
export interface Atlas {version:string;sex?:'male'|'female';region?:string;source?:string;scope?:string;parts:Part[];concepts:Concept[];chunks:{url:string;bytes:number;gzip?:string;gzipBytes?:number}[];triangles:number}
